...
Estamos interesados en poder identificar qué temas existen en las revistas para hombres y mujeres cada uno pero quisiéramos evitar la lectura de la totalidad de los artículos. Por eso vamos a usar LDA en este corpus.
...
Vemos que este conjunto de datos contiene documentos (cada uno de ellos un discurso) y términos (palabras). Observe que esta matriz de documento-término de ejemplo es 100% dispersa (segurmente, hay un efecto redondeo allí), lo que significa que casi todas las entradas en esta matriz son cero. Cada entrada distinta de cero corresponde a una determinada palabra que aparece en un determinado documento.
...
La función `tidy`conviritó el modelo a un formato de un tópico-término por fila. Para cada combinación, el modelo calcula la probabilidad de que ese término se genere a partir de ese tópico. Por ejemplo, el término "abajo" tiene una probabilidad de 0.02482 d ser generado a partir del tema 2. Esta valor baja sensiblemente en el resto de los tópicos.
...
Por ejemplo, el modelo estima que  alrededor del 45% de las palabras en el documento 9 se generaron a partir del tema 3. O menos del 1% de las palabras del documento 1 se generan a partir del tópico 3.
