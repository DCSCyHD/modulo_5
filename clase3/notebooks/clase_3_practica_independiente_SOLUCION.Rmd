---
title: "Clase 3. Práctica independiente"
output: 
  html_document:
    toc: TRUE
    toc_float: TRUE

date: "`r format(Sys.time(), '%d %B, %Y')`"
---

Vamos a hacer el mismo ejercicio para Piglia con la editorial Anagrama pero con la "Biblioteca del Pensamiento Socialista" de la [Editorial Siglo XXI](https://sigloxxieditores.com.ar/).

```{r, results='hide'}
library(rvest)
library(tidyverse)
library(httr)
```

## 1. Identificar la/s páginas que queremos descargar
Leemos o buscamos el perfil del autor. 
(Si no hay catálogo, ¿dónde busco?)
```{r}
### 
```

## 2. Generar lista de links
Traemos los links de los libros
```{r}
links <- c()
for (i in 1:4){
        link <- paste0("https://sigloxxieditores.com.ar/catalogo/page/", i, "/?coleccion=filter_colecciones=sociologia-y-politica")

        
        soc <- read_html(link)

        libros <- soc %>% 
          html_elements("div.product-wrapper")%>% 
          html_elements("a.product-image-link") %>%
          html_attr("href")

        links <- append(links, libros)
}

```

## 3. Scrapeamos todo...
Hacemos una función que inserte en una tabla el título, le autore, resumen y precio: 
```{r}

scrap_book_sxxi <- function(l){
        
        libro <- read_html(links[1])
        
        tit <-  libro %>%
                        html_elements("h1") %>%
                        html_text2() %>%
                        first()
        
        aut <-  libro %>%
                        html_elements("a.author") %>%
                        html_elements("a") %>%
                        html_text()
        
        res <- libro %>%
                        html_elements("div.woocommerce-product-details__short-description") %>%
                        html_text2()


        prec <- libro %>%
                        html_elements("div.price") %>%
                        html_elements("bdi") %>%
                        html_text()
        
        return(
          tibble(
                titulo=tit,
                autore=aut,
         #       sinopsis=res,
                precio=prec
        ))

        
}
```


```{r}
scrap_book_sxxi(links[1])
```


Y ahora la corremos en un loop:

```{r}
books <- tibble(
                titulo=character(),
                autore=character(),
                sinopsis=character(),
                precio=character()
                )

for (l in links[1:2]){
                b <- scrap_book_sxxi(l=l)
                print(b)
                books <- books %>% bind_rows(b)
}
```

```{r}
books
```

