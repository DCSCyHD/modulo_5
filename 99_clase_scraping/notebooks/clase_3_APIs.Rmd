...
- Devuelven la información en formato JSON. Consiste en objetos ordenados por pares de `nombre:valor` separados por comas y puestos entre llaves `{}`. Para leer este formato vamos a necesitar el paquete `rjson`. En el ejemplo que vamos a ver no va a ser necesario porque la API tiene un parámetro de consulta que permite obtener la información en formato csv.
...
```{r}
bra <- GET("https://api.worldbank.org/pip/v1/pip?country=BRA&year=all&povline=2&fill_gaps=false&format=csv")

bra <- read_csv(rawToChar(bra$content))
...
bra %>% 
        ...
        geom_line() +
        labs(x='Año',
             y='% población c/ menos de 2 U$S diarios') +
        theme_minimal() +
        theme(axis.text.x = element_text(size = 8)) +
        facet_wrap(~reporting_level)
```
...
Vamos a usar herramientas de autenticación en la última clase cuando trabajemos con [gemini](https://ai.google.dev/) el LLM de Google.
